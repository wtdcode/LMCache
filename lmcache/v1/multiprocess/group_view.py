...
def slice_block_ids_per_group(
    ...
    Args:
        ...
        group_tokens_per_block: Each group's tokens-per-paged-chunk, in
            engine-group order. Every positive value must divide both range
            endpoints; ``0`` marks a non-positional group that yields an
            empty list.
    ...
    for engine_group_idx, tokens_per_block in enumerate(group_tokens_per_block):
        if tokens_per_block <= 0:
            # Non-positional group (e.g. a per-request ring buffer that is not
            # prefix cacheable): it holds no KV for a token range.
            sliced.append([])
            continue
