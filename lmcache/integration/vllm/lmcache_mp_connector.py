...
class LMCacheMPConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        needs_checkpoints = any(
            getattr(g.kv_cache_spec, "prefix_cacheable", True) is not False
            and (
                "MambaSpec" in {c.__name__ for c in type(g.kv_cache_spec).__mro__}
                or "SlidingWindow"
                in "".join(c.__name__ for c in type(g.kv_cache_spec).__mro__)
            )
            for g in vllm_groups
        )
